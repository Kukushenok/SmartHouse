    ...
    def update(self,tick):
        if self.job:
            if self.job["type"] == "light":
                self.light_transparency += (self.job["transparency_set_to"]-self.job["transparency_was"])/self.job["total_time"]
            elif self.job["type"] == "hot":
                self.hot_transparency += (self.job["transparency_set_to"]-self.job["transparency_was"])/self.job["total_time"]
            elif self.job["type"] == "cold":
                self.cold_transparency += (self.job["transparency_set_to"]-self.job["transparency_was"])/self.job["total_time"]
            elif self.job["type"] == "temperature":
                self.temperature += (self.job["transparency_set_to"] - self.job["transparency_was"]) / self.job["total_time"]
                if self.temperature<0:
                    self.cold_transparency = self.temperature*-5
                    self.hot_transparency = 0
                else:
                    self.hot_transparency = self.temperature*5
                    self.cold_transparency = 0
